...
const ReviewLegalDocumentClt = {
    ...
    getReviewLegalDocument: async (req, res) => {
        try {
            ...
            let termsConditions = [];
            let privacyPolicy = [];
            let lastUpdatedDes = ReviewLegalDocument[0].disclaimer[0].lastUpdated
            let disclaimerh1 = ReviewLegalDocument[0].disclaimer[0].disclaimer.content[0];
            let disclaimerh2 = ReviewLegalDocument[0].disclaimer[0].ExternalLinks.content[0];
            let disclaimerh3 = ReviewLegalDocument[0].disclaimer[0].MediacalInfo.content[0];
            let disclaimerh4 = ReviewLegalDocument[0].disclaimer[0].MeTestonomial.content[0];
            disclaimer.push({lastUpdated : lastUpdatedDes},disclaimerh1, disclaimerh2, disclaimerh3, disclaimerh4)
            let lastUpdatedTerms = ReviewLegalDocument[0].termsConditions[0].lastUpdated
            let termsConditions1 = ReviewLegalDocument[0].termsConditions[0].agreementTearms.content[0];
            let termsConditions2 = ReviewLegalDocument[0].termsConditions[0].intelectualProperty.content[0];
            let termsConditions3 = ReviewLegalDocument[0].termsConditions[0].userPresentaion.content[0];
            let termsConditions4 = ReviewLegalDocument[0].termsConditions[0].userRegistration.content[0];
            termsConditions.push({lastUpdated : lastUpdatedTerms} ,termsConditions1, termsConditions2, termsConditions3, termsConditions4)
            let lastUpdatedPolicy = ReviewLegalDocument[0].privacyPolicy[0].lastUpdated
            let privacyPolicy1 = ReviewLegalDocument[0].privacyPolicy[0].Privacy.content[0];
            let privacyPolicy2 = ReviewLegalDocument[0].privacyPolicy[0].Summery.content[0]
            let privacyPolicy3 = ReviewLegalDocument[0].privacyPolicy[0].SummeryOfKeyPoints.content[0];
